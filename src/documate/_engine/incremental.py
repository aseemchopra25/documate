...
_MAX_PARSE_WORKERS = int(
    os.environ.get("CRG_PARSE_WORKERS", str(min(os.cpu_count() or 4, 8)))
)
...
def _load_ignore_patterns(repo_root: Path, extra: Sequence[str] = ()) -> list[str]:
    """Load ignore patterns from .code-review-graphignore file.

    `extra` appends caller-supplied patterns (documate passes its config's
    skip_dirs, converted to fnmatch form) so one knob governs both the graph
    and the docs."""
    patterns = list(DEFAULT_IGNORE_PATTERNS) + list(extra)
...
_RECURSE_SUBMODULES = os.environ.get("CRG_RECURSE_SUBMODULES", "").lower() in (
    "1",
    "true",
    "yes",
)
...
def get_changed_files(repo_root: Path, base: str = "HEAD~1") -> list[str]:
    ...
    if detect_vcs(repo_root) == "svn":
        return _get_svn_changed_files(
            repo_root, base if _SAFE_SVN_REV.match(base) else None
        )
    # Git path
...
def collect_all_files(
    ...
    Args:
        ...
        extra_ignore: Additional ignore patterns appended to the defaults.
    """
    ignore_patterns = _load_ignore_patterns(repo_root, extra_ignore)
    ...
    for rel_path in candidates:
        ...
        try:
            ...
            continue
        if len(str(full_path)) > 1000 or any(
            len(p.encode()) > 255 for p in full_path.parts
        ):
...
def full_build(
    ...
    """
    parser = CodeParser(repo_root)
    files = collect_all_files(repo_root, recurse_submodules, extra_ignore)
