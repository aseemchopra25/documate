...
class GraphStore:
    ...
    def __init__(self, db_path: str | Path) -> None:
        ...
        try:
            ...
            # NORMAL is crash-safe under WAL (never corrupts — at worst it loses
            # the last commit on power loss), and the graph is a disposable,
            # rebuildable index, so we trade that sliver of durability for far
            # fewer fsyncs across a full build's per-file commits.
            ...
            # A failed init (foreign schema, locked file) must not leak the
    ...
    def store_file_nodes_edges(
        ...
        try:
            ...
            self._insert_nodes(nodes, fhash)
    ...
    def _insert_nodes(self, nodes: list[NodeInfo], fhash: str) -> None:
        """Batch-upsert a file's nodes in one ``executemany``.

        Same insert-or-update-on-``qualified_name`` semantics as
        :meth:`upsert_node`, minus the per-node ``SELECT id`` round trip (the
        store path discards the id). Within-batch duplicates resolve via
        ON CONFLICT, so a re-emitted symbol still collapses to one row.
        """
        if not nodes:
            return
        now = time.time()
    ...
    def _insert_edges(self, edges: list[EdgeInfo]) -> None:
        """Batch-insert a file's edges in one ``executemany``.

        Called only after :meth:`remove_file_data` has cleared the file's
        edges, so no committed row can collide. Exact duplicates the parser may
        emit (same kind/source/target/file/line) are collapsed here, last-wins
        — matching the old per-edge upsert — while distinct call sites (same
        target, different line) are preserved.
        """
        if not edges:
            return
        now = time.time()
    ...
    def resolve_bare_call_targets(self) -> int:
        ...
        for edge in bare_edges:
            ...
            if len(candidates) == 1:
                ...
                src_file = (
                    src_qn.split("::", 1)[0] if "::" in src_qn else edge["file_path"]
                )
                imported_files = import_targets.get(src_file, set())
                imported = [
                    c for c in candidates if c.split("::", 1)[0] in imported_files
                ]
                if len(imported) == 1:
