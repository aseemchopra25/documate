...
_STAMP = GENERATED_STAMP  # every generated page opens with it; the anchor scanner keys off it
...
_AGENT_BEGIN = "<!-- documate:begin -->"
_AGENT_END = "<!-- documate:end -->"
...
def _agent_pointer(ctx: Context) -> list[str]:
    ...
    block = (
        f"{_AGENT_BEGIN}\n"
        f"## Code map (generated by documate)\n\n"
        ...
        f"callees) live in `{ddir}/architecture/`. Regenerate with `documate`;\n"
        f"never hand-edit those pages.\n"
        f"{_AGENT_END}"
    )
    changed: list[str] = []
    for name in ("AGENTS.md", "CLAUDE.md"):
        ...
        if _AGENT_BEGIN in text and _AGENT_END in text:
            head, _, rest = text.partition(_AGENT_BEGIN)
            _, _, tail = rest.partition(_AGENT_END)
            new = head + block + tail
        else:
            new = text.rstrip("\n") + "\n\n" + block + "\n"
        if new != text:
...
def run(ctx: Context, diff: bool = False, quiet: bool = False) -> int:
    ...
    theirs = [
        ...
        and not p.read_text(encoding="utf-8", errors="replace").startswith(_STAMP)
    ]
    if theirs:
        for rel in theirs:
