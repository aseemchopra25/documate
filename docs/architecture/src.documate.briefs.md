...
**called by** `_drift_brief`, `_undoc_briefs`  ·  **calls** `_used`

### `_abs_q(ctx: Context, tgt: dict) -> str`
`src/documate/briefs.py:138`
...
### `emit(ctx: Context, base: str, direct: list[dict], out_dir: Path, undocumented: str='changed') -> list[dict]`
`src/documate/briefs.py:375`
...
**calls** `_drift_brief`, `_module_briefs`, `_slug`, `_undoc_briefs`, `_xrefs`
